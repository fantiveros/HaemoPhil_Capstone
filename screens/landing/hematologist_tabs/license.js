...
import { styles } from '../patient_tabs/profile_screens/styles'
import Icon from 'react-native-vector-icons/Feather'
import * as ImagePicker from 'expo-image-picker';
import { ref, uploadBytes, getStorage} from 'firebase/storage';
import { UploadLicenseImage } from './common/service';
...
export class License extends Component {
  render() {
    return <LicenseScreen user={this.props.user}/>
  }
}

const LicenseScreen = ({user}) => {
  ...
  const uploadImage = async () => {
    if(image)
    {
      ...
      const storageRef = ref(storage, `licenses/${filename}`);
      ...
      .then(async res => {
        const isCompleted = await UploadLicenseImage(user, `licenses/${filename}`)
        if(isCompleted)
        {
